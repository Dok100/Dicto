...
public final class AppSettings: ObservableObject {
    ...
    @Published public var transcriptionEngine: TranscriptionEngine {
        didSet {
            UserDefaults.standard.set(transcriptionEngine.rawValue, forKey: StorageKey.Defaults.transcriptionEngine)
        }
    }
    ...
    public var openAIApiKey: String {
        ...
        set { KeychainService.shared.save(
            newValue.trimmingCharacters(in: .whitespacesAndNewlines),
            forKey: StorageKey.Keychain.openAIApiKey) }
    }
    ...
    public init() {
        ...
        self.ollamaBaseURL = d.string(forKey: StorageKey.Defaults.ollamaBaseURL) ?? "http://localhost:11434"
        self.ollamaModel = d.string(forKey: StorageKey.Defaults.ollamaModel) ?? "qwen2.5:32b"
        self.ollamaPrompt = d.string(forKey: StorageKey.Defaults.ollamaPrompt) ?? AppSettings.defaultPrompt
        self
            .transcriptionEngine = TranscriptionEngine(rawValue: d
                .string(forKey: StorageKey.Defaults.transcriptionEngine) ?? "") ?? .whisper
        self.whisperModel = WhisperModel(rawValue: d.string(forKey: StorageKey.Defaults.whisperModel) ?? "") ?? .largev3
        self.previewEnabled = d.object(forKey: StorageKey.Defaults.previewEnabled) as? Bool ?? false
        self.soundFeedbackEnabled = d.object(forKey: StorageKey.Defaults.soundFeedbackEnabled) as? Bool ?? true
        self.customStyles = d.data(forKey: StorageKey.Defaults.customStyles)
            .flatMap { try? JSONDecoder().decode([CustomStyle].self, from: $0) }
            ?? []
        self.whisperLanguage = d.string(forKey: StorageKey.Defaults.whisperLanguage)
            .flatMap(WhisperLanguage.init) ?? .german
        self.dictationShortcut = d.data(forKey: StorageKey.Defaults.dictationShortcut)
            .flatMap { try? JSONDecoder().decode(ShortcutConfig.self, from: $0) }
            ?? .defaultDictation
        self.transformShortcut = d.data(forKey: StorageKey.Defaults.transformShortcut)
            .flatMap { try? JSONDecoder().decode(ShortcutConfig.self, from: $0) }
            ?? .defaultTransform
        self.llmProvider = LLMProvider(rawValue: d.string(forKey: StorageKey.Defaults.llmProvider) ?? "") ?? .ollama
        self.openAIModel = d.string(forKey: StorageKey.Defaults.openAIModel) ?? "gpt-4o-mini"
        self.openAIBaseURL = d.string(forKey: StorageKey.Defaults.openAIBaseURL) ?? "https://api.openai.com/v1"
        ...
        if let legacyEnabled = d.object(forKey: StorageKey.Defaults.ollamaEnabledLegacy) as? Bool {
            if !legacyEnabled { self.llmProvider = .disabled }
